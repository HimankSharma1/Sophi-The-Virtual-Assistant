...
class MainWindow:
    ...
    def StartTheWindow(self, geometry, title, win_bg, resizable=False):
        ...
        self.outPut=Text(self.root, bg="black", font=("Times", 12), cursor='arrow', fg= 'light green')
        self.outPut.config(yscrollcommand=self.scrollbar.set, state="disable")
        self.scrollbar.config(command=self.outPut.yview)
        self.scrollbar.pack(side=RIGHT,fill=Y)
        self.outPut.place(x=268, y=4, height=528, width=510)
        ...
        self.clock_label=Label(self.root, font=("Times", 32), bg="black", fg="#36F01A")
        self.clock_label.place(x=15, y=475)
        def CleanTheWin():
        ...
        def clock():
            t=str(datetime.datetime.now().strftime("%I:%M:%S %p"))
            self.clock_label.config(text=t)
            self.clock_label.after(300, clock)
        clock()
        ...
        from SophiaManageDBMS import deleteContact, uploadContacts
        self.database.add_command(label="Upload Contact", command=uploadContacts)
        ...
        from SophiaManageDBMS import add_file, add_link, delete_file, delete_web, add_folder, delete_folder , add_pin
        ...
        self.AddData.add_command(label="Add Home Auomation", command=add_pin)
        ...
        self.AddData.add_command(label="Delete System Folder", command=delete_folder)
        ...
        def out2(text, say=True, output=True):
            ...
            if output==True:
                ...
                pass
        def Activate():
            text="First speak the hotword 'Sophia' to activate the assistant and wait for the beep sound. After listening the beep sound tell your command."
            out2(text)
